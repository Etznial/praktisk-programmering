...
/* A must be square and invertable, A=QR where R should be an upper triangular matrix, Q should be and orthoganol or unitary matrix -> Q^T=Q^(-1). The columbs of Q form an orthonalmal basis*/
	...
	public static vector solveR(matrix R, vector b){ /* Løser kun højretriangulær matrix*/
		int n = b.size;
	...
	public static vector solve(matrix Q, matrix R, vector b){ /* løser Ax=b A=QR => QRx=b => Rx=QTb << */
		matrix QT = Q.transpose();
		vector QTb = QT*b;
		int n = b.size;
		vector solutions = solveR(R,QTb);
		return solutions; 
	}
	...
	public static matrix inverseR(matrix R){
		int n = R.size1;
